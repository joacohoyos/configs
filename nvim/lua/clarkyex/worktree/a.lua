...
Worktree.on_tree_change(function(op, path, upstream)
    ...
    if op == Worktree.Operations.Switch and is_pe_api() then
        ...
        add_window(
            'eyewear',
            'run',
            vim.env.PE_PATH .. '/docker',
            string.format(
                './scripts/start.bash.clark -s %s -f %s',
                vim.g.pe_api_branch,
                vim.g.pe_app_branch
            )
        )
    ...
    if op == Worktree.Operations.Create and is_pe_api() then
        os.execute(
            string.format('cp -r $PE_PATH/api/develop/docker/local $PE_PATH/api/%s/docker', dir)
        )
    ...
    if op == Worktree.Operations.Create and is_pe_app() then
        os.execute(
            string.format('cp -r $PE_PATH/app/develop/docker/local $PE_PATH/app/%s/docker', dir)
        )
    ...
    if op == Worktree.Operations.Create and is_kiddom_api() then
        os.execute(
            string.format(
                'cp -r $KIDDOM_GO_PATH/master/application/config/app.json %s',
                dir .. '/application/config/app.json'
            )
        )
        os.execute(
            string.format(
                'cp -r $KIDDOM_GO_PATH/master/application/config/test.json %s',
                dir .. '/application/config/test.json'
            )
        )
    end
    ...
    if op == Worktree.Operations.Create and is_kiddom_app() then
        os.execute(
            string.format('cp -r $KIDDOM_PATH/web/master/node_modules $KIDDOM_PATH/web/%s', dir)
        )
        os.execute(
            string.format(
                'cp -r $KIDDOM_PATH/web/master/config/local.config.js $KIDDOM_PATH/web/%s/config/local.config.js',
                dir
            )
        )
    end
