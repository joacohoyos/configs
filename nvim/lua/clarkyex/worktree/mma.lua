...
function MMA.on_tree_change(op, path, upstream, dir) 
    if op == Worktree.Operations.Switch and is_mma_api() then
      ...
      Common.add_window("mma", "run-api", get_dir("api/" .. Common.get_branch(dir)), "./stop.sh && ./start.sh up")
    ...
    if op == Worktree.Operations.Switch and is_mma_app() then
      ...
      Common.add_window("mma", "run-app", get_dir("app/" .. Common.get_branch(dir)), "npm run dev")
